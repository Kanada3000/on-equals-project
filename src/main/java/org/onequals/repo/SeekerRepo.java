...
public interface SeekerRepo extends JpaRepository<Seeker, Long> {
    ...
    @Query("SELECT c FROM Seeker c WHERE c.approved = false")
    List<Seeker> findUnapproved(Sort sort);

    @Query("SELECT c FROM Seeker c join c.city cit WHERE c.approved = false AND " +
    ...
    List<Seeker> findUnapproved(String field, String value, Sort sort);
